# This is a slight modification of my original Linear_Generate_AdversaryEA.py script
# If you want to save time, I recommend you run that script instead
...
# This generates an adversarial image (classified as 6) based on the target_image passed in; maybe will break this
# into a class in the future
def breed_adversary (t_img, session, n):
    population = np.zeros((POPULATION_N, 784))
    for i in POPULATION_R:                      # Seeding the initial image population as the target image (two)
        population[i] = t_img

        sample = population[1]                  # Test sample from population

    # Print the image
    sample.shape = (28, 28)
    plt.imshow(sample, cmap='gray')
    plt.savefig("Results/Original{0}.png".format(n))

    children = np.zeros((POPULATION_N, 784))    # Empty child population


    for i in GENERATION_R:
        scores = session.run(y, feed_dict={x: population})  # Score the entire population = target class % confidence
        ...
        for j in range(POPULATION_N/2):
            ...
            parent1 = tournament([population[selection[0]], population[selection[1]], population[selection[2]]], t_img,
                                 [scores[selection[0], 6], scores[selection[1], 6], scores[selection[2], 6]], 1, i)
            selection = rand.sample(POPULATION_R, TOURNAMENT_S)
            parent2 = tournament([population[selection[0]], population[selection[1]], population[selection[2]]], t_img,
                                 [scores[selection[0], 6], scores[selection[1], 6], scores[selection[2], 6]], 1, i)
    ...
    # Take a sample from out final population, (I just took the first since they all should have converged to
    # be similar) and checks the classification
    sample = population[1]
    delta = sample - t_img
    sample.shape = (1, 784)
    result = sess.run(y, feed_dict={x: sample})
    print ("New: ", result)

    # Print images
    delta.shape = (28, 28)
    plt.imshow(delta, cmap='gray')
    plt.savefig("Results/Delta{0}.png".format(n))
    sample.shape = (28, 28)
    plt.imshow(sample, cmap='gray')
    plt.savefig("Results/Adversarial{0}.png".format(n))
...
while count < 10:
    ...
    if (np.argmax(result) == 2) and (np.argmax(y_) == 2):
        count += 1
        print count
        print ("Predicted :", result)
        print ("Actual :", y_)
        breed_adversary(image, sess, count)
